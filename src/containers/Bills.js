...
export default class { 
  ...
  constructor({ document, onNavigate, store, localStorage }) { 
    ...
    // Stocke l'objet document (ou DOM => représentation de la structure de la page web chargée dans le navigateur) dans la classe pour un usage ultérieur 
    ...
    // Crée une nouvelle instance de la classe 'Logout' pour gérer la déconnexion de l'utilisateur 
  }

  // Méthode appelée lorsqu'on clique sur le bouton "Nouvelle Note de Frais"
  handleClickNewBill = () => { 
  ...
  handleClickIconEye = (icon) => { 
    ...
    // Calcule la largeur de l'image à afficher dans la modale en fonction du conteneur/modale
    //  Utilisation de  pour sélectionner l'élément HTML avec l'ID modaleFile (conteneur) et obtient sa largeur en pixels, qu'on multiplie par 0.5, ce qui signifie qu’on veut une largeur d'image équivalente à 50% de celle du conteneur. La fonction Math.floor() arrondit le résultat au nombre entier inférieur 
    $('#modaleFile').find(".modal-body").html(`<div style='text-align: center;' class="bill-proof-container"><img width=${imgWidth} src=${billUrl} alt="Bill" /></div>`) 
    // jQuery ($) - Injecte le code HTML nécessaire pour afficher l'image de la note de frais dans la modale
    $('#modaleFile').modal('show') 
    // jQuery ($) - Affiche la modale contenant l'image de la facture
  }
  ...
  getBills = () => {
    ...
    if (this.store) {
      // Appel à `this.store.bills().list()` pour récupérer la liste des factures depuis le backend ou API
      return this.store
        .bills()
        .list() // `list()` renvoie une promesse qui, une fois résolue, contient un tableau de factures
        .then(snapshot => {
          // Une fois que la promesse est résolue avec les factures, le `then` est exécuté avec `snapshot` (tableau contenant les factures) => données récupérées depuis le backend
          const bills = snapshot
            // Filtrage des factures pour ne conserver que celles qui ont un `name`, un `amount`, et un `status` définis. Cela permet de s'assurer que seules les factures complètes sont manipulées
            ...
            .map(doc => {
              try {
                ...
                return {
                  ...doc, // Récopie toutes les propriétés de `doc` dans le nouvel objet facture. Cela permet de conserver les valeurs originales de toutes les propriétés de doc dans le nouvel objet, sauf celles choisies pour modification explicite ci-après 
                  date: formatDate(doc.date), // La date est formatée pour être affichée de manière lisible dans l'UI (format français)
                  rawDate: doc.date, // On conserve la date brute (non formatée) pour effectuer des opérations de tri
                  status: formatStatus(doc.status) // Le statut de la facture est également formaté pour être affiché de manière plus lisible (en français)
                };
              } catch (e) {
                ...
                return {
                  ...
                  rawDate: doc.date, // Si le formatage échoue, on conserve la date brute
          ...
          bills.sort((a, b) => new Date(b.rawDate) - new Date(a.rawDate));
